...
StartSection(run := true) {
    ...
    ; Restart the Tick timer in case it was disabled by Complete()
    SetTimer(Tick, 200)
}
Tick() {
    ...
    ; 1. Bounds Guard: Prevent out-of-bounds array access when lesson completes
    if (idx < 1 || idx > lesson.Length)
        return
        
    ; 2. State Guard: Abort early if paused or transitioning (Logical order fix)
    if (paused || transitioning)
        return
        
    ; 3. During a @reminder, pause all timer logic
    if (lesson[idx].HasProp("isReminder") && lesson[idx].isReminder)
        return
...
Complete() {
    ...
    ; --- Stop main timer to prevent background execution (Fix 3) ---
    SetTimer(Tick, 0)
